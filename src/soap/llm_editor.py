...
_EDIT_INSTRUCTIONS = (
    ...
    "anything you omit is deleted. To CHANGE an existing note, use one update_note "
    "on its [N] in place — never delete_note it and add_note a replacement, which "
    "throws away the note's identity and its ICD code. Propose no ICD codes — that is not yours to "
