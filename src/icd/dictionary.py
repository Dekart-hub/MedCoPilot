...
class IcdEntry:
    """One classifier record: a code and its canonical title."""
...
def load_dictionary(path: Path) -> list[IcdEntry]:
    """Load ``(code, name)`` entries from a JSON array file."""
    records: list[Any] = json.loads(path.read_text(encoding="utf-8"))
    return [IcdEntry(code=record["code"], name=record["name"]) for record in records]
